...
public class ImplementExecutionOrder {
    ...
    public static List<ExpandedComponentInstanceSymbol> exOrder(ExpandedComponentInstanceSymbol inst) {
        dependencies.clear();
        s = 0;
        b = 0;
       ...
       /* for (ExpandedComponentInstanceSymbol symbol : dependencies.keySet()) {
        ...
        ExpandedComponentInstanceSymbol instTagged = tagExOrder(inst);
        return getExecutionOrder(instTagged);
    }
     ...
     */
    private static ExpandedComponentInstanceSymbol tagExOrder(ExpandedComponentInstanceSymbol inst) {
...
//      }
            ...
            if ((subInst.getSubComponents().isEmpty() && subInst.getIncomingPorts().isEmpty()
                ...
                for (ConnectorSymbol c : connects) {
                    ...
                    if (inst2.getIncomingPorts().contains(pt)) {
                        ...
                        dependencyPortDeletion((ExpandedComponentInstanceSymbol) inst.getEnclosingScope().getSpanningSymbol().get(), pt);
                    }
                    //delete dependency of port in target component
                    if (!inst2.getSubComponents().isEmpty()) {
                        dependencyPortDeletion(inst2, pt);
                    } else {
                        tagExOrderBranch(inst2, inst);
                    }
                }
            }
        }
     ...
     */
    private static ExpandedComponentInstanceSymbol dependencyPortDeletion(ExpandedComponentInstanceSymbol inst, PortSymbol p) {
        Collection<ConnectorSymbol> connects = inst.getConnectors().stream()
     ...
     */
    private static ExpandedComponentInstanceSymbol dependencyPortsDeletion(ExpandedComponentInstanceSymbol inst) {
        Collection<ConnectorSymbol> connects = inst.getConnectors().stream()
     ...
     */
    private static List<ExpandedComponentInstanceSymbol> getExecutionOrder(ExpandedComponentInstanceSymbol inst) {
        Map<ExecutionOrder, ExpandedComponentInstanceSymbol> exOrder = new HashMap<ExecutionOrder, ExpandedComponentInstanceSymbol>();
        exOrder = exOrderRecursion(exOrder, inst);
